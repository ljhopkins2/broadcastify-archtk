...
1. **Copy and modify the config files.** The system needs two config files to run:
    - `config.ini`
        ...
        - Edit the `MP3_OUT_PATH` value to point to where you want to store `mp3` files. Note that the directory _must exist before using the archive_, or you'll get an error
