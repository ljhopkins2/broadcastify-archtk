...
1. **Copy and modify the config files.** The system needs two config files to run:
    - `config.ini`
        ...
        - Edit the `MP3_OUT_PATH` value to point to where you want to store `mp3` files. Note that the directory MUST EXIST before usin the archive, or you'll get an error
